...
technologies (**Atera**, **VisiumHD**, **Xenium MM/biomarkers**,
**Xenium v1/breast+100**, **Xenium 5k**, **StrataMap**) on the same
...
One spatial scatter plot per sample (all 8, fixed order: StrataMap
grades 1-3, VisiumHD, Atera, Xenium 5k, Xenium MM, Xenium v1), points
colored by RCTD `first_type`. This is the most direct look at whether
each technology recovers the tissue’s known spatial organization
(e.g. tumor regions, stroma) at all, before any quantitative metric.
...
Cell-type proportions per sample, all 8 in one row, cell types ordered
by their overall abundance summed across every sample (same order in
every panel, so panel-to-panel comparison is direct).
...
Five panels sharing one technology axis: N genes, N cells, N
counts/cell, N counts/gene/cell, and bioconservation (ARI between
...
(“w2”) - how much of each cell’s expression the model attributes to a
second, distinct cell type. Higher/wider w2 distributions suggest more
segmentation-driven mixing of neighboring cells’ transcripts,
independent of any spatial-neighborhood test (that’s figure 7). Samples
ordered by increasing median score.
...
One point per technology: median transcripts/cell/gene on its own full
panel (y) vs. on the 353-gene shared panel axis (x), log-scaled, with a
dashed identity line and IQR error bars in both directions. Points above
the identity line detect more signal per gene on their own panel than on
the shared axis; point size reflects panel size at self. This isolates
whether a technology’s sensitivity advantage (or disadvantage) is about
panel size/design specifically, independent of the cell-type recovery
metrics in the other figures.
